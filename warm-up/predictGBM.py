...
def add_ratios_features (df) :
    ...
    df [ 'BUREAU_ACTIVE_CREDIT_TO_INCOME_RATIO' ] = df [ 'BUREAU_ACTIVE_AMT_CREDIT_SUM_SUM' ] / df [
        'AMT_INCOME_TOTAL' ]
...
def kfold_lightgbm_sklearn (data , categorical_feature = None) :
    ...
    for n_fold , (train_idx , valid_idx) in enumerate ( folds.split ( df [ predictors ] , df [ 'TARGET' ] ) ) :
        ...
        clf = GradientBoostingClassifier ( **{ **params , **LIGHTGBM_PARAMS } )
        ...
        #               categorical_feature = categorical_feature )  # , verbose=400, early_stopping_rounds=EARLY_STOPPING,

        oof_preds [ valid_idx ] = clf.predict_proba ( valid_x , num_iteration = clf.best_iteration_ ) [ : , 1 ]
        sub_preds += clf.predict_proba ( test [ predictors ] , num_iteration = clf.best_iteration_ ) [ : ,
                     1 ] / folds.n_splits
...
def get_previous_applications (path , num_rows = None) :
    ...
    active_pay_agg [ 'INSTALMENT_PAYMENT_DIFF' ] = active_pay_agg [ 'AMT_INSTALMENT' ] - active_pay_agg [
        'AMT_PAYMENT' ]
...
def get_pos_cash (path , num_rows = None) :
    ...
    df [ 'POS_COMPLETED_BEFORE_MEAN' ] = df.apply ( lambda x : 1 if x [ 'POS_COMPLETED_BEFORE_MEAN' ] > 0
                                                                    and x [ 'POS_LOAN_COMPLETED_MEAN' ] > 0 else 0 ,
                                                    axis = 1 )
    ...
    pos_agg = pd.merge ( pos_agg , gp_mean [ [ 'SK_ID_CURR' , 'LATE_PAYMENT_SUM' ] ] , on = 'SK_ID_CURR' ,
                         how = 'left' )
...
def get_installment_payments (path , num_rows = None) :
    ...
    # Last loan features
    g = parallel_apply ( gp , installments_last_loan_features , index_name = 'SK_ID_CURR' ,
...
def main (debug: bool = False , initialDataPopulating: bool = False) :
    num_rows = 30000 if debug else None

    if (initialDataPopulating) :
...
if __name__ == "__main__" :
    ...
    with timer ( "Pipeline total time" ) :
        main ( debug = False , initialDataPopulating = False )
